...
const Footer = () => {
  return (
      ...
      justify-between flex-wrap md:mt-0 mt-10'>
        {footerLinks.map((footerLink)=>(
          ...
          ss:my-0 my-4 min-w-[150px]'>
            <h4 className='text-white font-poppins font-medium text-[18px]
            leading-[27px] mb-5'>
              {footerLink.title}
            </h4>
            <ul className='list-none'>
              {footerLink.links.map((link,index)=>(
                ...
                hover:text-secondary cursor-pointer ${index!==link.length?"mb-3":"mb-0"}`}>
                  {link.name}
                </li>
              ))}
            </ul>
    ...
    <div className='w-full flex justify-between items-center md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3F3E45]'>
      <p className='text-white font-poppins font-normal text-center text-[18px]
            leading-[27px]'>
      ...
      <div className='flex flex-row md:mt-0 mt-6'>
        {socialMedia.map((social,index)=>(
          ...
          src={social.icon}
          alt={social.id}
          className={`h-[21px] w-[21px] object-contain cursor-pointer
